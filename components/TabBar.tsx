...
const TabBar = ({ state, descriptors, navigation }: BottomTabBarProps) => {
  ...
  // Animation values
  const tabPositions = [useSharedValue(0), useSharedValue(0), useSharedValue(0)];
  const tabScales = [useSharedValue(1), useSharedValue(1), useSharedValue(1)];
  ...
  return (
    <>
      {state.routes.map((route: any, index: number) => {
        ...
        useEffect(() => {
          if (isFocused) {
            tabScales[index].value = withSpring(1.1, { damping: 10 });
          } else {
            tabScales[index].value = withSpring(1, { damping: 10 });
          }
        }, [isFocused]);
        ...
        return (
          <TouchableOpacity
            ...
            style={[
              styles.tabbarItem,
              route.name === 'addImage' && styles.addTabItem,
            ]}
            onLayout={(e) => {
